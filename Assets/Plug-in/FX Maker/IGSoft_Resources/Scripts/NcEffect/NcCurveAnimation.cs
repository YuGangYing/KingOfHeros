...
//		Debug.Log("NcCurveAnimation.Start " + transform.parent.name);
	...
	float fElapsedTime;
	public bool IgnoreScaleTime = false;
	void LateUpdate()
	{
// 		Debug.Log("NcCurveAnimation.Update " + transform.parent.name);
		...
		{
			if (GetEngineTime(IgnoreScaleTime) < m_fStartTime + m_fDelayTime)
				return;
// 			m_fDelayTime = 0;
			InitAnimationTimer();
			if (GetComponent<Renderer>())
...
//		float fElapsedTime	= m_Timer.GetTime();
		if(IgnoreScaleTime){
			if(lastFrameTime>0)fElapsedTime +=  Time.realtimeSinceStartup - lastFrameTime;//YingYuGang
		}else{
		...
		if(IgnoreScaleTime)lastFrameTime = Time.realtimeSinceStartup;//YingYuGang
		UpdateAnimation(fElapsedRate);
	}

	public static float GetEngineTime(bool ignoreScaleTime)
	{
		if(ignoreScaleTime)
		{
