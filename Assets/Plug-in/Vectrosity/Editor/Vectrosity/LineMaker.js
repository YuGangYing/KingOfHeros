...
class LineMaker extends ScriptableWizard {
	...
	function Initialize () {
		...
		for (i = 0; i < pointObjects.Length; i++) {
			...
			pointObjects[i].localScale = Vector3(pointScale*objectScale.x, pointScale*objectScale.y, pointScale*objectScale.z);
	...
	function SetPointScale () {
		...
		for (po in pointObjects) {
			po.localScale = Vector3(pointScale*objectScale.x, pointScale*objectScale.y, pointScale*objectScale.z);
		}
	}
