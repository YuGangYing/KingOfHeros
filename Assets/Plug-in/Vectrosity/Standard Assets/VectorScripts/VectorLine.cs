...
public class VectorLine {
	...
	public int minDrawIndex {
		...
		set {
			...
			if (!m_continuous && m_minDrawIndex % 2 != 0) {	// No odd numbers for discrete lines
				m_minDrawIndex++;
			}
	...
	void RebuildMesh (bool use2Dlines, int pointsLength) {
		...
		Color[] colors = null;
		if (lineColors != null) {
			colors = SetColor (lineColors[0], continuous? LineType.Continuous : LineType.Discrete, pointsLength, isPoints);
		}
		if (lineWidths.Length > 1) {
	...
	void BuildMesh (int pointsLength, bool use2Dlines, bool useSegmentColors, Color[] colors) {
		...
		int end = 0;
		if (!m_isPoints) {
			end = continuous? pointsLength-1 : pointsLength/2;
		}
		else {
			end = pointsLength;
		}
		...
		idx = 0;
		if (!m_isPoints) {
			end = continuous? (pointsLength-1)*4 : pointsLength*2;
		}
		else {
			end = pointsLength*4;			
		}
