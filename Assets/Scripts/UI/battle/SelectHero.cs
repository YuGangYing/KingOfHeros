...
public class SelectHero : UITweener
{
    ...
    void OnClick()
    {
        ...
        if (isSelected)
        {
			DataManager.getBattleUIData().SetCurHero(-1 ,-1);
        }
        else
        {
            UILabel id = PanelTools.Find<UILabel>(gameObject, "idHero");
			DataManager.getBattleUIData().SetCurHero(Index, int.Parse(id.text));
        }
        ...
        if (battle != null)
        {
            ...
            if (skillState != null)
            {
                if (isSelected)
                {
                    skillState.SetActive(false);
                }
                else
                {
                    skillState.SetActive(true);
                }   
            }  
        } 

        GameObject Heros = transform.parent.gameObject;
        foreach (Transform child in Heros.transform)
        {
            SelectHero sh = child.gameObject.GetComponent<SelectHero>();

            if (sh != null)
            {
                if (sh.Index != Index || sh.isSelected == true)
                {
                    sh.isSelected = false;
                    sh.PlayReverse();
                }
