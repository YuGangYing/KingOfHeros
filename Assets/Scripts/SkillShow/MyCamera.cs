...
public class MyCamera : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (UICamera.hoveredObject != null || !isEnable)
        {
            return;
        }
        ...
        bool rotate = MyInputUtils.GetButtonSafe(MouseRotateButton, false);
        bool mouseLook = MyInputUtils.GetButtonSafe(MouseLookButton, false);
        ...
        realDistance -= MyInputUtils.GetAxisRawSafe(ZoomAxis, 0f) * ZoomSpeed;
        ...
        {
            targetYaw = MyInputUtils.SignedAngle(offset.normalized, -Target.forward, Vector3.up);
        ...
        {
            targetYaw += (MyInputUtils.GetAxisRawSafe(YawAxis, 0f) * RotationMouseSpeed);
            targetPitch -= (MyInputUtils.GetAxisRawSafe(PitchAxis, 0f) * RotationMouseSpeed);
        ...
        {
            targetYaw = MyInputUtils.SignedAngle(offset.normalized, -Target.transform.forward, Vector3.up);
...
public static class MyInputUtils
{
    // ÏòÁ¿v1Óëv2
