...
//	public static PutBuild GetInstance;
	...
	void Start ()
	{
		...
		for (int i = 0; i < WWWNetwork.WWWNetworkManager.GetInstance.model.building_list.Count; i++) {
			BuildingModel model = WWWNetwork.WWWNetworkManager.GetInstance.model.building_list[i];
			if(model.type == (int)DataMgr.BuildType.WALL || model.type == (int)DataMgr.BuildType.CELLAR || model.type == (int)DataMgr.BuildType.MONUMENT){
				continue;
			}
			OnPutBuild (model);
		}
...
//         }
	...
	public Build OnPutBuild (BuildingModel info)
	{
		GameObject objBuild = GetNewBuild ((int)info.type, info.level, new Pos2D (info.pos_x, info.pos_y));
		if (objBuild) {
			Build buildTemp = objBuild.GetComponent<Build> ();
			buildTemp.m_idBuilding = (uint)info.id;
			buildTemp.m_idBuildingType = (uint)info.type;
			buildTemp.m_cbAreaWidth = 1;
			buildTemp.m_cbAreaHigh = 1;
			buildTemp.m_cbLev = (byte)info.level;
			buildTemp.m_cbState = (byte)info.status;
			buildTemp.m_u32LevyTime = (uint)System.DateTime.Now.Millisecond;
			buildTemp.CreateBuildPop ();
			//buildTemp.CleanRoundTree();
//			if (info.pos_x != (float)buildTemp.m_buildFound)
//			{
//				// 更正服务器位置
...
//                objBuild = (GameObject)Object.Instantiate(prefab2);
				objBuild = KOH.ResourcesManager.GetInstance.GetBuildingObejct (strPrefab);
