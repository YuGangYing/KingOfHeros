...
public class GridItem : MonoBehaviour
{
    ...
    public SolidityGrid m_pathGrid;
    ...
    public eValueType SetValue = eValueType.Inside;

    public void Rasterize()
    {
        m_pathGrid = transform.parent.GetComponent<TerrainLayer>().m_grid;
        if (m_pathGrid == null)
            return;

        int numObstructedCells = 0;
        int[] obstructedCells = GetObstructedCells(m_pathGrid, out numObstructedCells);
        ...
        for (int i = 0; i < numObstructedCells; i++)
        {
            ...
            {
                m_pathGrid.SetSolidity(obstructedCellIndex, (short)SetValue);
            }
        }

        if (numObstructedCells != 0)
        {
            transform.parent.GetComponent<TerrainLayer>().ToGridData();
        }
    }
    ...
    void OnGUI()
    {
        ...
        {
            IsSave = false;
            TerrainLayer layer = transform.parent.GetComponent<TerrainLayer>();
            if (layer == null)
                return;

            layer.AddToList = true;
        }
    }
}
