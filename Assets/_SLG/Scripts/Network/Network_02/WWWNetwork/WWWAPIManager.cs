...
namespace WWWNetwork
{
	public class WWWAPIManager : SingleMonoBehaviour<WWWAPIManager>
	{
		...
		{
			CreateBuildingAPI api = WWWNetworkManager.GetInstance.gameObject.GetOrAddComponent<CreateBuildingAPI> ();
			api.data = new ChangeBuildingModel ();
		...
		public void SendUpgradeBuilding(uint idBuilding){
			UpgradeBuildingAPI api = WWWNetworkManager.GetInstance.gameObject.GetOrAddComponent<UpgradeBuildingAPI> ();
			api.data = new ChangeBuildingModel ();
			api.data.id = (int)idBuilding;
			api.Send ((WWW www) => {
				Debug.Log (www.text);
